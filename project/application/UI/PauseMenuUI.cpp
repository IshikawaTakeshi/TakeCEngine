...
void PauseMenuUI::Initialize(SpriteManager* spriteManager) {
	...
	itemNames_.resize(magic_enum::enum_count<PauseMenuResult>() - 1); // Noneを除く
	for (int i = 0; i < (int)itemNames_.size(); ++i) {
		itemNames_[i] = StringUtility::EnumToString(static_cast<PauseMenuResult>(i + 1)); // Noneをスキップ
	}
	...
	operatorInstructions_ = GetSprite("OperatorInstructions");
...
void PauseMenuUI::ExecuteCurrent() {
	...
	case 3: result_ = PauseMenuResult::OperatorInstructions; break;
	default: result_ = PauseMenuResult::None; break;
	}
	// ポーズメニューUIを閉じる
	if (result_ != PauseMenuResult::OperatorInstructions) {
		Close();
	}
}
...
void PauseMenuUI::UpdateVisual() {
	...
	// 操作説明の表示
	if (currentIndex_ == static_cast<int>(PauseMenuResult::OperatorInstructions) - 1) {
		operatorInstructions_->SetIsActive(true);
		cursor_->SetIsActive(false);
	} else {
		operatorInstructions_->SetIsActive(false);
		cursor_->SetIsActive(true);
	}
}
...
//	結果の消費
//============================================================================
PauseMenuUI::PauseMenuResult PauseMenuUI::ConsumeResult() {
