...
//============================================================================
/// <summary>
/// Bulletに関する生成、更新、破棄を一元管理するクラスです。
/// </summary>
class BulletManager {
public:
	//=====================================================================
	...
	// 初期化
	void Initialize(TakeC::Object3dCommon* object3dCommon, size_t size);
	// 開放処理
	void Finalize();
	// 更新
	void Update();
	...
	Bullet* ShootBullet(const Vector3& weaponPos, const Vector3& targetPos,
		const Vector3& targetVel, float speed, float power,
		CharacterType type);
	Bullet* ShootBullet(const Vector3& weaponPos, const Vector3& direction,
		float speed, float power, CharacterType type);
	...
	/// <param name="power">      ダメージ量</param>
	/// <param name="type">        キャラクタータイプ</param>
	void ShootMissile(BaseWeapon* ownerWeapon, VerticalMissileInfo vmInfo,
		float speed, float power, CharacterType type);

	void ShootBazookaBullet(const Vector3& weaponPos, const Vector3& targetPos,
		const Vector3& targetVel, float speed, float power,
		CharacterType type);
	void ShootBazookaBullet(const Vector3& weaponPos, const Vector3& direction,
		float speed, float power, CharacterType type);
...
private:
	TakeC::Object3dCommon* object3dCommon_ = nullptr; // 3Dオブジェクト共通クラス
