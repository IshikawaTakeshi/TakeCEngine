...
// 　初期化処理
//===================================================================================
void Rifle::Initialize(Object3dCommon* object3dCommon,
	...
	weaponState_.attackInterval =
		rifleInfo_.burstShotInfo.kInterval; // 連射の間隔を攻撃間隔に設定
	weaponState_.bulletCount = weaponData_.config.maxMagazineCount;
	weaponState_.remainingBulletCount =
		weaponData_.config.maxBulletCount; // 残弾数を最大弾数に設定
...
void Rifle::Update() {
	...
	if (weaponState_.isReloading) {
		...
		if (weaponState_.reloadTime <= 0.0f) {
			...
			weaponState_.bulletCount =
				weaponData_.config
				.maxMagazineCount; // 弾数をマガジン内の弾数にリセット
			weaponState_.remainingBulletCount -=
				weaponData_.config.maxMagazineCount; // 残弾数を更新
		}
	}
	...
	if (burstShotState_.isActive) {
		...
		if (burstShotState_.intervalTimer <= 0.0f && burstShotState_.count > 0) {
			...
			if (CanMoveShootable()) {
				direction += targetVelocity_ *
					0.1f; // ターゲットの速度を加味（調整係数0.1fは適宜変更）
				direction.Normalize();
			}
			...
			if (ownerObject_->GetCharacterType() == CharacterType::PLAYER) {
				// プレイヤーの弾として発射
				auto* bullet = bulletManager_->ShootBullet(
					object3d_->GetCenterPosition(), direction,
					weaponData_.config.bulletSpeed, weaponData_.config.power,
					CharacterType::PLAYER_BULLET);

				if (bullet) {
					bullet->SetBreakStunPower(weaponData_.config.breakStunPower);
					bullet->SetBreakStunDecayInterval(
						weaponData_.config.breakStunDecayInterval);
				}

			} else if (ownerObject_->GetCharacterType() == CharacterType::ENEMY) {
				// エネミーの弾として発射
				auto* bullet = bulletManager_->ShootBullet(
					object3d_->GetCenterPosition(), targetPos_, targetVelocity_,
					weaponData_.config.bulletSpeed, weaponData_.config.power,
			...
			if (weaponState_.bulletCount <= 0) {
				...
				weaponState_.reloadTime =
					weaponData_.config.maxReloadTime; // リロード時間をリセット
			}

			burstShotState_.count--;
			if (burstShotState_.count > 0) {
				burstShotState_.intervalTimer =
					rifleInfo_.burstShotInfo.kInterval; // 次の弾発射までの間隔を設定
			} else {
	...
	if (parentSkeleton_ && !parentJointName_.empty()) {
		// Matrix4x4 characterWorldMatrix =
		// ownerObject_->GetObject3d()->GetWorldMatrix();
		auto jointWorldMatrixOpt = parentSkeleton_->GetJointWorldMatrix(
			parentJointName_, ownerObject_->GetObject3d()->GetWorldMatrix());
...
void Rifle::UpdateImGui() {
	...
	ImGui::Text("required Charge Time: %.2f",
		weaponData_.config.requiredChargeTime);
	ImGui::Text("Is Charging: %s", weaponState_.isCharging ? "Yes" : "No");
	ImGui::SliderFloat("##Rifle::Bullet Speed", &weaponData_.config.bulletSpeed,
		100.0f, 1000.0f);
	ImGui::Separator();
...
void Rifle::ChargeAttack() {
	...
	if (weaponState_.chargeTime >= weaponData_.config.requiredChargeTime) {
		...
		burstShotState_.count =
			rifleInfo_.burstShotInfo.kMaxBurstCount; // 3連射のカウントを初期化
