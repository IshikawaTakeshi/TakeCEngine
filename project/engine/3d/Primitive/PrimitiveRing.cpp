...
// 初期化
//============================================================================
void PrimitiveRing::Initialize(DirectXCommon* dxCommon, SrvManager* srvManager) {
	dxCommon_ = dxCommon;
...
// 終了処理
//============================================================================
void PrimitiveRing::Finalize() {
	for (auto& pair : datas_) {
		pair.second->mesh.vertexBuffer_.Reset();
		pair.second->mesh.indexBuffer_.Reset();
		// material は unique_ptr なので自動解放
	}
	datas_.clear();
}
...
// リングデータの生成
//============================================================================
uint32_t PrimitiveRing::Generate(float outerRadius, float innerRadius, const std::string& textureFilePath) {
...
// 頂点データ作成
//============================================================================
void PrimitiveRing::CreateVertexData(RingData* ringData) {

	// この Ring 固有の頂点数を計算
	uint32_t vertexCount = ringData->subDivision * kVerticesPerSegment;
	ringData->vertexCount = vertexCount;
	...
	// 頂点データ生成
	const float radianPerDivide = (2.0f * std::numbers::pi_v<float>) / static_cast<float>(ringData->subDivision);
...
// マテリアル作成
//============================================================================
void PrimitiveRing::CreateMaterial(const std::string& textureFilePath, RingData* ringData) {
...
// 描画処理（パーティクル用）
//============================================================================
void PrimitiveRing::DrawParticle(PSO* pso, UINT instanceCount, uint32_t handle) {

	// インスタンス数が0の場合は早期リターン（D3D12警告対策）
	if (instanceCount == 0) {
		return;
	}
...
// 描画処理（オブジェクト用）
//============================================================================
void PrimitiveRing::DrawObject(PSO* pso, uint32_t handle) {
...
// データ取得
//============================================================================
PrimitiveRing::RingData* PrimitiveRing::GetData(uint32_t handle) {
	auto it = datas_.find(handle);
	if (it == datas_.end()) {
		return nullptr;
	}
...
// マテリアル色設定
//============================================================================
void PrimitiveRing::SetMaterialColor(uint32_t handle, const Vector4& color) {
	auto it = datas_.find(handle);
	if (it == datas_.end()) {
		return;
	}
